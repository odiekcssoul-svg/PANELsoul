-- ============================================================
-- FUNCIÓN RPC PARA CANJEAR CÓDIGO — SECURITY DEFINER
-- Corre con permisos de admin, bypasea RLS
...
BEGIN
  ...
  -- 4. Buscar inventario disponible
  ...
  IF v_client_id IS NULL THEN
    INSERT INTO gift_clients (owner_id, name, phone, email, browser, os, device, tags, last_seen)
    VALUES (v_code.owner_id, p_client_name, p_client_phone, p_client_email, p_browser, p_os, p_device, '{}', v_now)
    RETURNING id INTO v_client_id;
  ELSE
    UPDATE gift_clients
    SET owner_id = COALESCE(owner_id, v_code.owner_id),
        last_seen = v_now,
  ...
  INSERT INTO gift_redemptions (
    ...
    code_used, product, account_email, account_password,
    browser, os, device, status, redeemed_at
  ) VALUES (
    v_code.owner_id, v_client_id, v_code.id, v_inventory.id,
    UPPER(TRIM(p_code)), v_inventory.product, v_inventory.email, v_inventory.password,
    p_browser, p_os, p_device, 'completed', v_now
  );
...
-- Permitir que anon ejecute esta función
